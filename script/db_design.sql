...
CREATE TABLE IF NOT EXISTS `travel_memories`.`user` (
  ...
  `u_regtime` INT(13) NOT NULL COMMENT '注册时间',
  `u_lasttime` INT(13) NOT NULL COMMENT '最近登录时间',
  PRIMARY KEY (`u_id`))
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `travel_memories`.`event_point` (
  ...
  `ep_time` INT(13) NOT NULL DEFAULT 0 COMMENT '事件发生时间',
  `ep_cretime` INT(13) NOT NULL DEFAULT 0 COMMENT '事件点创建时间',
...
CREATE TABLE IF NOT EXISTS `travel_memories`.`event_point_text_info` (
  ...
  `u_id` INT(10) NOT NULL,
  `ep_ti_lasttime` INT(13) NOT NULL DEFAULT 0 COMMENT '最近更改时间',
...
CREATE TABLE IF NOT EXISTS `travel_memories`.`event_point_media_info` (
  `ep_mi_id` INT(15) NOT NULL AUTO_INCREMENT,
